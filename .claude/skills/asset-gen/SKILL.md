...
Generate PNG images (Gemini or xAI Grok) and GLB 3D models (Tripo3D) from text prompts. These are paid APIs — every call costs real money. Tools live at `.claude/skills/asset-gen/tools/`; run from the project root and keep runtime-loaded outputs under `src/assets/`.
...
python3 .claude/skills/asset-gen/tools/asset_gen.py image \
  --prompt "the full prompt" -o src/assets/img/car.png
```
...
Read `.claude/skills/asset-gen/rembg.md`. Key rule: **never prompt for a "transparent background"** (the generator bakes a checkerboard) — prompt a solid color, then matte it out.
...
```bash
python3 .claude/skills/asset-gen/tools/asset_gen.py glb  --image ref.png -o model.glb     # 30¢ default / 60¢ --quality hd
python3 .claude/skills/asset-gen/tools/asset_gen.py rig  --image ref.png -o rigged.glb    # +25¢, biped only
python3 .claude/skills/asset-gen/tools/asset_gen.py retarget --rigged rigged.glb \
...
- A timeout in `glb`/`rig`/`retarget` does **not** mean server failure. The task id is already saved in the `<output>.tripo.json` sidecar. **Do not resubmit — that double-charges.** Resume for free instead:
  ```bash
  python3 .claude/skills/asset-gen/tools/asset_gen.py resume -o model.glb
  ```
...
|------|-------------|------|------|------|
| car | sedan with spoiler | 4m long | src/assets/glb/car.glb | 37¢ |
